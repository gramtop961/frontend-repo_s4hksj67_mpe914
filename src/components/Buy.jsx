...
import { MapPin, Filter, Car, DollarSign, Search } from 'lucide-react'
const baseUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'

function FilterBar({ onChange }){
  const [f, setF] = useState({ location:'', car_type:'', min_price:'', max_price:'', q:'', mode:'sale' })
  ...
  return (
    <div className="p-4 rounded-xl bg-white/5 border border-white/10 hover:border-cyan-400/40 transition-colors">
      ...
      <div className="mt-3 flex items-center justify-between">
        <p className="text-emerald-300 font-semibold">${car.sale_price ?? '--'}</p>
      ...
      <a href={`/buy?car=${car.id}`} className="mt-3 block text-center px-4 py-2 rounded bg-emerald-600 hover:bg-emerald-500">Buy</a>
    </div>
  )
}
...
function Buy(){
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-extrabold mb-4">Buy Cars</h1>
